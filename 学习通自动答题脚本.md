...
---
## 描述 ##
__此脚本UI界面几乎全部借助AI,仅支持单选,多选题的自动答题,需额外付费__

<img src="https://summit-oss.oss-cn-beijing.aliyuncs.com/picture.png"/>

* Token获取地址为<a href="https://globalai.vip" target="_blank">这里</a>，如果修改，那么AI选择可能也会随之变少，仅支持修改两种。
* 代码中的提示词建议不要擅自修改
* 假如你的令牌来自deepSeek,就将代码中的url_dp改为参考地址(注释包含),令牌填写为你的令牌即可
* 自动提交会根据答题质量来判断,如果置信度(结果的可靠性)达到8(1-10),就会触发 

## 使用方法 

__浏览器(任意,支持扩展插件)__ => __设置-扩展-获取扩展-搜索脚本猫__ => __脚本猫界面,获取脚本-创建脚本__=>__复制下面代码,粘贴__=>__键入`Ctrl+S`保存__

<p style = "color :red ">一但进入学习通(网页)答题界面,就会出现弹窗助手,输入密钥,选择模型后点击`开始答题`即可进行 </p>

## 不稳定性

__此脚本依赖于学习通的DOM结构,一但后续官方出现页面重组形式的设计,大概率会出问题__
...
(function () {
  "use strict";
  //如果要修改你的api地址,url_dp:deepseek :一般为https://api.deepseek.com/v1/chat/completions ; url_gpt :chatGpt:一般为https://api.openai.com/v1/chat/completions
  var url_base = "https://globalai.vip/v1/chat/completions"; //这个url_base即为参考地址,可以在这个网址申请购买一个实惠的令牌
  var url_dp = "https://globalai.vip/v1/chat/completions";
  var url_gpt = "https://globalai.vip/v1/chat/completions";
  ...
  function toggleTextOfSubBtn(autoSub) {
    let isAuto = false;
    autoSub.addEventListener("click", () => {
      isAuto = !isAuto;
      autoSub.innerText = isAuto ? "取消自动提交" : "自动提交";
    });
  }
  ...
  const tips =
    "只需要回答答案格式为:题号+选项内容+答案+置信度(一定正确的概率,1-10),例如:'1 A 人工智能 10',如果为多选,则格式为:'1 AC null 9',中间用空格分隔,任何回答不可以输出此提示语,如果遇到类似简答题,直接返回答案";
...
.btn-auto-submit {
  ...
  font-size: 10px;
  cursor: pointer;
  transition: all 0.3s ease; /* 缓慢变色效果 */
}
...
.btn-auto-submit:active {
  background: rgba(16, 185, 129, 0.3);
      ...
      <div class="answer-helper-body">
        <div class="form-group">
          ...
          <div class="btn-group">
                ...
                <button id="autoSubmit" class="btn-auto-submit">自动提交</button>
        </div>
    ...
    const autoSub = document.querySelector("#autoSubmit");
    toggleTextOfSubBtn(autoSub);
  ...
  function addListenerAll(question, main) {
    ...
    const completeBtn = document.querySelector("#submitFocus .completeBtn");
    ...
    autoSubmit.addEventListener("click", () => (isAutoSubmit = !isAutoSubmit));
        ...
        for (let i = 0; i < question.length; i++) {
          ...
          let each = choices
            ? choices.querySelectorAll(
                "[aria-label^='A'],[aria-label^='B'],[aria-label^='C'],[aria-label^='D']"
              )
            : null;
          let choiceOf;

          let resEnd = await getAnswer(choice, key, questionli.innerText);
          let result = resEnd.split(/\s+/); //["1","AC","null","100"]
          let btn2;
            ...
            for (let j = 0; j < 4; j++) {
              ...
              if (choiceOf) {
                ...
                if (btn && btn.innerText == Quelist[count]) {
                  btn.click();
                  confidenceLevel += parseInt(result[result.length - 1]);
                  count++;
                  continue;
                }
              }
  ...
  function Submit(completeBtn, queLength, confidenceLevel) {
    let interval = 100,
        timeout = 5000;
    if (confidenceLevel && completeBtn && confidenceLevel / queLength > 8) {
      try {
        setTimeout(() => {
          completeBtn.click();
          //2秒之后点击确认提交
          ...
          const tempId = setInterval(() => {
            ...
            let confirm = document.querySelector("#popok");
            if (confirm) {
              confirm.click();
              clearInterval(tempId);
            } else if (curTime - startTime > timeout) {
              clearInterval(tempId);
             console.error("自动提交超时,请手动提交");
            }
          }, interval);
        }, 2000);
      } catch (e) {
        return e.message ? e.message : "没有提交成功,请手动提交";
      }

      return "已自动提交";
    } else return "自动提交失败,请手动提交";
  }
